...
1. ⏳ 验证 Cursor 对 MDC 格式的支持情况（前置条件）
2. 📋 完成 MDC 格式迁移（如 Cursor 支持）或保持 Markdown 格式
...
**目标**：开发工具自动将完整版转换为精简版，减少 80% 人工工作。

#### 转换规则引擎

**功能描述**：

转换规则引擎是一个自动化工具，用于将完整版规则转换为精简版，减少 80% 人工工作。预期准确率 70-80%，所有转换结果必须经过人工复审。

**核心转换规则**：

1. **删除详细背景**：移除"背景"、"为什么"等解释性章节
2. **精简示例**：保留前 N 个 Good 示例和 1 个 Bad 示例
3. **删除重复说明**：识别并移除重复的说明内容
4. **压缩段落为列表**：将长段落转换为简洁的列表项
5. **删除主观描述**：移除"优雅"、"漂亮"等主观性词汇

**转换流程**：

- 输入：完整版规则文件（`.md`）
- 处理：应用所有转换规则
- 输出：精简版规则文件（自动生成到对应目录）
- 后处理：删除多余空行，格式化输出
...
| 规则文件 | 完整版 | 自动精简版 | 人工精简版 | 准确率 |
|---------|-------|----------|----------|-------|
| 01-general.md | 850 tokens | 210 tokens | 180 tokens | 90% |
| 02-testing.md | 620 tokens | 190 tokens | 150 tokens | 85% |
| 03-security.md | 780 tokens | 165 tokens | 140 tokens | 92% |
